...
async function register(data){
  ...
  var condition = { emailId : data.emailId , phoneNumber : data.phoneNumber },
  ...
  var authToken = generate.authToken({ _id:user._id, name :user.name });
   return {
          responseCode : 200,
          responseMessage : "Your account created successfully",
          result : {name:user.name ,authToken : authToken }
      }
    }
}

async function login(data){
  var condition = { phoneNumber : data.phoneNumber , password: data.password };
  ...
  if(!success){
    ...
    var authToken = generate.authToken({ _id:success._id, name :success.name });
      return {
          responseCode : 200,
          responseMessage : "Your account created successfully",
          result : { name:success.name ,authToken : authToken }
    }
  }
}

async function forgotPassword(data){
   var success = await db.user.findOne({emailId : data.emailId},'password name emailId');
   if(success){
